  ...
  fluidRow(class = "myRow1", 
    ...
    column(
      ...
      selectizeInput(
        ...
        choices = c("Both", "Female", "Male"), selected = 1
      ),
      selectizeInput(
        inputId = 'age', label = 'Age', width = "100%", multiple = TRUE,
        choices = aa, selected = 1,
      ...
      selectizeInput(
        inputId = 'edu', label = 'Educational Attainment', width = "100%", multiple = TRUE,
        choices = ee, selected = 1,
        options = list(placeholder = 'Not available')
      ),
      checkboxInput(
        inputId = 'alledu', label = 'Include all age groups',
        value = FALSE
      )
    ),
    column(
