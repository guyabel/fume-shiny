...
d <- read_csv("./data/benchmark/pop.csv")
##
##Indicators
##
nn <- unique(d$dest)
names(nn) <- countrycode(sourcevar = nn, origin = "iso3c", destination = "country.name")
ss <- c("Both", "Male", "Female")
aa <- paste0(seq(0, 95, 5), "-", seq(0, 95, 5) + 4)
aa <- c("Total", aa, "100+")
ee <- unique(d$edu)
bb <- unique(d$birth)
yy <- unique(d$year)
